...
{
    public partial class frm_BaoCaoHangTonKho : Form
    {
        public frm_BaoCaoHangTonKho()
        {
            InitializeComponent();
        }

        private void frm_BaoCaoHangTonKho_Load(object sender, EventArgs e)
        {
            QL.Text = QL.Text + frm_DangNhap.Tenquanly;

            DataTable dt = new DataTable();
            dt = Class.cl_load.loaddl("select KHOHANG.MaHang,TenHang,SLNhap,SLGiao,TonKho from KHOHANG inner join HANGHOA on HANGHOA.MaHang = KHOHANG.MaHang");
            reportViewer1.LocalReport.ReportEmbeddedResource = "BCTT_QLKH.Danh_Muc_Bao_Cao.Report_TonKho.rdlc";
