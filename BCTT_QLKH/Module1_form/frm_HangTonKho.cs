...
{
    public partial class frm_HangTonKho : Form
    {
        public frm_HangTonKho()
        {
            InitializeComponent();
        }

        private void frm_HangTonKho_Load(object sender, EventArgs e)
        {
            QL.Text = QL.Text + frm_DangNhap.Tenquanly;

            DataTable dt;
            dt = Class.cl_load.loaddl("select KHOHANG.MaKho,TenKho,KHOHANG.MaHang,TenHang,SLNhap,SLGiao,TonKho from KHOHANG inner join HANGHOA on KHOHANG.MaHang = HANGHOA.MaHang inner join KHO on KHO.MaKho=KHOHANG.MaKho");
            ...
            dataGridView1.Columns[5].HeaderText = "Số Lượng Giao";
