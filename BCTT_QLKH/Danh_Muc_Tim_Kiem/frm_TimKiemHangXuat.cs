...
{
    public partial class frm_TimKiemHangXuat : Form
    {
        DataTable dt = null;
        public frm_TimKiemHangXuat()
        {
            InitializeComponent();
        }
        public DataTable load_dl(string sql)
        {
            ...
            dgvHang.Columns[8].HeaderText = "Giá Giao";
            dgvHang.Columns[9].HeaderText = "Số Lượng Giao";
        ...
        private void frm_TimKiemHangXuat_Load(object sender, EventArgs e)
        {
            dt = load_dl("select PHIEUGIAOCHITIET.SoPG,NgayGiao,KHOHANG.MaKho,TenKho,DONDAT.MaKhach,TenKhach,KHOHANG.MaHang,TenHang,PHIEUGIAOCHITIET.GiaGiao,PHIEUGIAOCHITIET.SLGiao from KHOHANG inner join PHIEUGIAOCHITIET on PHIEUGIAOCHITIET.MaHang = KHOHANG.MaHang inner join HANGHOA on KHOHANG.MaHang = HANGHOA.MaHang inner join PHIEUGIAO on PHIEUGIAOCHITIET.SoPG = PHIEUGIAO.SoPG inner join DONDAT on DONDAT.SoDD = PHIEUGIAO.SoDD inner join KHACHHANG on KHACHHANG.MaKhach = DONDAT.MaKhach inner join KHO on KHOHANG.MaKho = KHO.MaKho");
        ...
        private void btnTim_Click(object sender, EventArgs e)
        {
            ...
            if (radSoPhieu.Checked == true) timkiem = "and PHIEUGIAOCHITIET.SoPG='" + txtSP.Text.Trim() + "'";
            if (rabKho.Checked == true) timkiem = " and KHOHANG.MaKho='" + txtMaKho.Text.Trim() + "'";
            if (rabNCC.Checked == true) timkiem = " and DONDAT.MaKhach='" + txtNCC.Text.Trim() + "'";
            dt = load_dl("select PHIEUGIAOCHITIET.SoPG,NgayGiao,KHOHANG.MaKho,TenKho,DONDAT.MaKhach,TenKhach,KHOHANG.MaHang,TenHang,PHIEUGIAOCHITIET.GiaGiao,PHIEUGIAOCHITIET.SLGiao from KHOHANG inner join PHIEUGIAOCHITIET on PHIEUGIAOCHITIET.MaHang = KHOHANG.MaHang inner join HANGHOA on KHOHANG.MaHang = HANGHOA.MaHang inner join PHIEUGIAO on PHIEUGIAOCHITIET.SoPG = PHIEUGIAO.SoPG inner join DONDAT on DONDAT.SoDD = PHIEUGIAO.SoDD inner join KHACHHANG on KHACHHANG.MaKhach = DONDAT.MaKhach inner join KHO on KHOHANG.MaKho = KHO.MaKho  " + timkiem);
